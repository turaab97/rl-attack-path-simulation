...
SUBNETS = {
    0: {"name": "Subnet 0: Internet", "color": "#E0E0E0", "y": 5.0,
        "hosts": []},
    1: {"name": "Subnet 1: DMZ", "color": "#BBDEFB", "y": 4.0,
        "hosts": [
            {"id": "(1,0)", "label": "Web Server\nLinux | HTTP, SSH\nValue: 0", "x": 1.5},
            {"id": "(1,1)", "label": "Email Gateway\nWindows | SMB, RDP\nValue: 0", "x": 3.5},
            {"id": "(1,2)", "label": "DNS Server\nLinux | SSH, HTTP\nValue: 0", "x": 5.5},
        ]},
    2: {"name": "Subnet 2: Corporate LAN", "color": "#FFF9C4", "y": 2.8,
        "hosts": [
    ...
    3: {"name": "Subnet 3: AI Infrastructure", "color": "#FFCDD2", "y": 1.6,
        "hosts": [
            {"id": "(3,0)", "label": "LLM API Server\nLinux | HTTP, SSH\nValue: 200", "x": 2.0},
            {"id": "(3,1)", "label": "Vector Database\nLinux | HTTP\nValue: 200", "x": 4.2},
            {"id": "(3,2)", "label": "Model Repository\nLinux | SSH\nValue: 150", "x": 6.2},
        ]},
    4: {"name": "Subnet 4: Data Lake", "color": "#B71C1C", "y": 0.4,
        "hosts": [
...
def generate_topology_diagram(
    ...
    for subnet_id, subnet in SUBNETS.items():
        if not subnet["hosts"]:
            ...
            ax.annotate(
                ...
                bbox=dict(boxstyle="round,pad=0.4", facecolor=subnet["color"],
        ...
        zone_rect = FancyBboxPatch(
            (zone_left, subnet["y"] - 0.45), zone_right - zone_left, 0.9,
            boxstyle="round,pad=0.1", facecolor=subnet["color"],
            edgecolor="#616161", linewidth=1.2, alpha=0.4, zorder=0,
        )
        ax.add_patch(zone_rect)
        ax.text(
            zone_left + 0.1, subnet["y"] + 0.35, subnet["name"],
            fontsize=8, fontweight="bold", color="#424242", va="top",
        )
        ...
        for host in subnet["hosts"]:
            ...
            ax.annotate(
                ...
                bbox=dict(
                    boxstyle="round,pad=0.3", facecolor=face_color,
    ...
    for src, tgt, services in CONNECTIONS:
        if src in host_positions and tgt in host_positions:
            ...
            ax.text(
                mid_x, mid_y, services,
                fontsize=5, color="#546E7A", ha="center", va="center",
    ...
    legend_items = [
        ...
        mpatches.Patch(facecolor="#FFCDD2", edgecolor="#616161", label="AI Infrastructure (High Value)"),
